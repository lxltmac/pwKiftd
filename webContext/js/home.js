...
var isImportingExcel = false;// 是否正在执行导入excel操作
	...
	$.ajax({
		...
		success : function(result) {
			...
			default:
				...
				$("#fim_creator").text(folderView.folder.folderCreatorName);
...
function showAccountView(folderView) {
	...
	if (authList != null) {
		...
		if (checkAuth(authList, "P")) {
		            $("#importAccountExcelButtonLi").removeClass("hide");
        			$("#importAccountExcelButtonLi").removeClass("disabled");
        			$("#importAccountExcelButtonLi a").attr("onclick",
        					"showImportAccountExcelModel()");
        		}
...
function showFolderTable(folderView) {
	...
	if (parentpath != null && parentpath != "null") {
		$("#foldertable")
				.append(
						"<tr onclick='returnPF()'><td><button onclick='' class='btn btn-link btn-xs'>返回</button></td><td class='hidden-xs'>--</td><td>--</td><td class='hidden-xs'>--</td><td>--</td></tr>");
	}
	var authList = folderView.authList;
	var aD = false;
	var aR = false;
...
function createFileRow(fi, aL, aD, aR, aO) {
	...
	fileRow = fileRow + "<td class='hidden-xs'>" + fi.fileCreatorName + "</td><td>";
	if (aL) {
		fileRow = fileRow
...
// 取消导入文件
function abortImportExcel() {
    if (xhr != null) {
			xhr.abort();
	}
	$('#importAccountExcelModal').modal('hide');
}


// 检查文件是否能够导入
function checkImportExcelFile() {
	if (isImportingExcel == false) {
		if (fs != null && fs.length > 0) {
			$("#importExcelFilepath").attr("disabled", "disabled");
			$("#ipbutton").attr('disabled', true);
			isUpLoading = true;
			repeModelList = null;
            ...
            var uploadfile = fs[0];// 获取要上传的文件
            var fname = uploadfile.name;
			uploadTargetFolder = locationpath;// 记录上传目标
            var formData = new FormData();// 用于封装文件数据的对象
            formData.append("file", uploadfile);// 将文件对象添加到FormData对象中，字段名为uploadfile
            formData.append("fname", fname);
            formData.append("folderId", uploadTargetFolder);
			...
			$
					.ajax({
						...
						dataType : 'text',
						data : formData,
						url : "homeController/checkImportFile.ajax",
                        processData: false,
                        contentType: false,
                        cache: false,
						success : function(result) {
							if (result == "mustLogin") {
								...
								switch (result) {
								case "errorParameter":
									showImportExcelAlert("提示：参数不正确，无法开始上传");
									break;
								case "noAuthorized":
									showImportExcelAlert("提示：您的操作未被授权，无法开始上传");
									break;
								case "errorEmpty":
                                    showImportExcelAlert("提示：文件类型为空，不能导入");
                                    break;
								case "errorContentType":
                                    showImportExcelAlert("提示：导入文本类型不正确，必须要为.xlsx, .xls类型的");
                                    break;
                                case "uploaderror":
									showImportExcelAlert("提示：出现意外错误，无法开始导入");
                                    break;
								case "uploadsuccess":
                                    $('#importAccountExcelModal').modal('hide');
                                    showFolderView(locationpath);
			...
			showImportExcelAlert("提示：您未选择任何文件，无法开始导入");
		}
	} else {
		showImportExcelAlert("提示：另一项上传文件或文件夹的任务尚未完成，无法开始导入");
	}
}

// 显示上传文件错误提示
function showImportExcelAlert(txt) {
	isUpLoading = false;
    $("#importExcelAlert").addClass("alert");
    $("#importExcelAlert").addClass("alert-danger");
	$("#importExcelFilepath").removeAttr("disabled");
	$("#importExcelAlert").show();
	$("#importExcelAlert").text(txt);
	$("#ipbutton").attr('disabled', false);
}


// 文件选中后自动回填文件路径
function showImportFilepath() {
	var filename = "";
	for (var i = 0; i < fs.length; i++) {
	...
	if (fs.length <= 1) {
		$("#selectExcelCount").text("");
	} else {
		$("#selectExcelCount").text("（共" + fs.length + "个）");
	}
	$("#importExcelFilepath").val(filename);
}
...
function sortbycn() {
	...
	setTimeout(function() {
		folderView.fileList.sort(function(v1, v2) {
			return order * v2.fileCreatorName.localeCompare(v1.folderCreatorName, "zh");
		});
		folderView.folderList.sort(function(v1, v2) {
			return order
					* v2.folderCreatorName.localeCompare(v1.folderCreatorName, "zh");
		});
		showFolderTable(folderView);
		$("#sortByCN").removeClass();
		if (order == -1) {
...
// 显示导入文件夹模态框
function showImportAccountExcelModel() {
    $("#importExcelAlert").removeClass("alert");
    $("#importExcelAlert").removeClass("alert-danger");
    $("#importExcelAlert").text("");
	$("#importAccountExcelModal").modal('show');
}
