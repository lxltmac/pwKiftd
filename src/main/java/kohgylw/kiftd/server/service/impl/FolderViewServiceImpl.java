...
public class FolderViewServiceImpl implements FolderViewService {
	...
	public String getFolderViewToJson(String fid, Boolean flag, final HttpSession session, final HttpServletRequest request) {
		...
		vf.setFolderCreatorName(ConfigureReader.instance().getFolderCreatorName(vf.getFolderCreator()));
		...
		List<Node> fileList = this.flm.queryByParentFolderIdSection(keyMap2);
		fileList.forEach(f -> {
			f.setFileCreatorName(ConfigureReader.instance().getFolderCreatorName(f.getFileCreator()));
		});
		...
		if (cr.authorized(account, AccountAuth.IMPORT_ACCOUNT_EXCEL, fu.getAllFoldersId(fid))) {
			authList.add("P");
		}
