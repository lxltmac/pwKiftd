...
import kohgylw.kiftd.server.listener.AccountListener;
...
public class FileServiceImpl extends RangeFileStreamWriter implements FileService {
	...
	private static final String ERROR_EMPTY = "errorEmpty";// 空错误标识
	private static final String ERROR_CONTENT_TYPE = "errorContentType";// 文本类型错误标识
	...
	@Override
	public String checkImportFile(HttpServletRequest request, HttpServletResponse response, final MultipartFile file) throws Exception {
		if(null == file || StringUtils.isEmpty(file.getOriginalFilename())){
			return ERROR_EMPTY;
		}
		String originalFilename = file.getOriginalFilename();
		boolean isExcelTypeFlag = originalFilename.endsWith(".xlsx") || originalFilename.endsWith(".xls");
		if(!isExcelTypeFlag){
		...
		final String folderId = request.getParameter("folderId");
		final String nameList = request.getParameter("namelist");
		System.out.println("nameList列表：" + nameList);
		System.out.println("folderId列表：" + folderId);
		System.out.println("getOriginalFilename列表：" + file.getOriginalFilename());
		// 目标文件夹合法性检查
		if (folderId == null || folderId.length() == 0) {
			return ERROR_PARAMETER;
		}
		...
		try {
			EasyExcel.read(file.getInputStream(), Account.class, new AccountListener(request, fs)).sheet().doRead();
		} catch (Exception e) {
			System.out.println("导入账户异常" + e);
			return UPLOADERROR;
		}
							...
							if (f.getFileName().equals(originalFileName)) {
								try {
									...
									if (account != null) {
										...
										f.setFileCreatorName(ConfigureReader.instance().getFolderCreatorName(account));
									} else {
	...
	public String doMoveFiles(HttpServletRequest request) {
		...
		// 以及是否具备操作权限
		if (!ConfigureReader.instance().authorized(account, AccountAuth.IMPORT_ACCOUNT_EXCEL, fu.getAllFoldersId(locationpath))) {
			return NO_AUTHORIZED;
		}
		...
		try {
			...
			for (final String id : idList) {
				...
				if (!ConfigureReader.instance().authorized(account, AccountAuth.IMPORT_ACCOUNT_EXCEL,
						fu.getAllFoldersId(node.getFileParentFolder()))) {
					return NO_AUTHORIZED;// 无操作权限
				}
				// 记录原始的文件路径，便于执行后记录日志
